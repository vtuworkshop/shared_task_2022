...
def parse_data(file_path, tokenizer, punctuation_dict,  sequence_len, token_style):
    """
    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        data_sentences = f.read().split('\n\n')

    for data_sent in data_sentences:
        lines = [line for line in data_sent.split('\n') if line.strip()]
        idx = 0
...
class ConllDataset(torch.utils.data.Dataset):
    def __init__(self, files, tokenizer, punctuation_dict, sequence_len, token_style, is_train=False, augment_rate=0.1,
        ...
        if isinstance(files, list):
            self.data = []
            for file in files:
                self.data += parse_data(file, tokenizer, punctuation_dict, sequence_len, token_style)
        else:
            self.data = parse_data(files, tokenizer, punctuation_dict, sequence_len, token_style)
